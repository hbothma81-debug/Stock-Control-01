...
- The Production queue and the laser screens load once per session (their effects fire only while empty) and again after a write on that same device. The header Refresh and the background beat reload neither, nor the stage settings. After a push or a change to a stage's settings, the floor tablets need a page reload, or they keep showing old holds.
